...
\end{split}
\end{equation}
przy czym $p$ to proporcja liczby osobników z~daną cechą do całości populacji w~połączonej próbie, zatem $p=(X_1+X_2)/(n_1+n_2)$. Ostatecznie otrzymujemy
\begin{equation}
...
Jeżeli zakładamy, że $Z_{X_1,X_2} \sim \mathcal{N}(0,1)$, mamy
\begin{equation}
P\left(-z_{1-\alpha/2}\leq \frac{p_1-p_2}{\sqrt{V_{X_1,X_2}}} \leq z_{1-\alpha/2}\right) = 1-\alpha,
\end{equation}
gdzie $z_{1-\alpha/2}$ to kwantyl rzędu $1-\alpha/2$ z rozkładu normalnego $\mathcal{N}(0, 1)$. Wobec tego, przedział ufności na poziomie $1-\alpha$ dla różnicy proporcji $p_1-p_2$ jest równy
\begin{equation}
\label{r2:ci}
...
W przypadku testu~E nie da się łatwo wyznaczyć przedziału ufności dla różnicy proporcji w~oparciu o~obszar krytyczny testu~E. Jednakże można numerycznie oszacować przedział ufności dla różnicy proporcji, korzystając ze wzoru~(\ref{estpvalue}).
...
Tak jak w przypadku testu~E, nie da się prosto wyznaczyć przedziału ufności dla różnicy proporcji $p_1-p_2$. Jednak również można wyliczyć ten przedział numerycznie, korzystając ze wzoru~(\ref{estpZb}).
